...
function renderChartFromState() {
  ...
  const filtered = filterSeriesByTimeframe(seriesObj, state.timeframe);
  const hadData = Object.values(seriesObj).some((s) => s.points.length > 0);
...
function renderChart(seriesObj, emptyMessage) {
  ...
  if (seriesList.length === 0 || !seriesList.some((s) => s.points.length > 1)) {
    svg.innerHTML = "";
    legend.innerHTML = "";
    empty.textContent = emptyMessage || "Not enough data yet — check back after the next hourly update.";
